const pokémon = Object.freeze([
    ...
    const bListPk = pokémon.filter( p => p.id%3==0 );
    ...
    const nintynine = pokémon.filter( p => p.id>99 );
    const nameee = pokémon.filter( p => p.types=="poison").map( p => p.name );
    const flying = pokémon.filter( p => p.types[1]=="flying").map( p => p.types[0] )
    const counter = pokémon.filter( p => p.types=="normal").length
